...
import {
    EClass
} from "./lib/Class/Entity.js";
import {
    stackParse
} from "./lib/Class/Error.js";
import {
    EVClass
} from "./lib/Class/Events.js";
import {
    CClass
} from "./lib/Command/Command.js";
import {
    CDBClass,
    IDBClass
...
import {
    emoji
} from "./lib/Lang/emoji.js";
import {
    parse
} from "./lib/Lang/parser.js";
import {
    text
} from "./lib/Lang/text.js";
import {
    Module
} from "./lib/Module/creator.js";
import {
    load
...
export function getRole(player) {
    if (player instanceof Player) player = player.name
    const e = new IDBClass(world, "roles").get(player)
    if (
        e !== "member" || e !== "admin" || e !== "moderator" || e !== "builder"
    ) return "member"
...
export function ThrowError(e, deleteStack = 0) {
    ...
    ? world.say(`${c}§4${type}: §c${message} §f${s}`): console.warn(`${type}: ${message} ${s}`);
}

...
class BattleRoyal {
  ...
  start(players) {
    try {
      ...
      XA.OLDDB.chests.set("br:" + Date.now(), chest);
      this.events = {
        tick: world.events.tick.subscribe(() => {
          ...
          if (this.time.tick <= 0) {
            ...
            for (const val of XA.OLDDB.chests.values()) {
        ...
        buttonPush: world.events.buttonPush.subscribe((data) => {
          ...
          if (XA.OLDDB.chests.get(id)) return;
          XA.OLDDB.chests.set(id, true);
          // Loot
          XA.Chat.debug(

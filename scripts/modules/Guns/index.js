...
import { Items, ItemStack, PlayerInventoryComponentContainer, world } from "@minecraft/server";
 ...
 * @returns {number}
 */
function getItemsCountClear(entity, id) {
	const count = entity.runCommand(`clear @s ${id}`).statusMessage.split(": ")[1].replace(/\D/gi, "");
 ...
 */
function findItem(entity, id, loreFirst = null) {
	// @ts-ignore
...
setTickInterval(() => {
	for (const pl of world.getPlayers()) {
		...
		if (gun) {
			...
			if (lore[0]) {
				XA.runCommand(
					`scoreboard players set "${pl.name}" ${gun.scoreName} ` + lore[0].split("/")[0].replace(/§./g, "") // §r
				);
			}
			...
			else XA.runCommand(`scoreboard players set "${pl.name}" ${gun.scoreName} 0`);
		}
	}
}, 0);
...
setTickInterval(async () => {
	for (const pl of world.getPlayers()) {
		...
		if (XA.Entity.getTagStartsWith(pl, "reload:")) {
			...
			const imho = findItem(pl, XA.Entity.getTagStartsWith(pl, "reload:") + "_empty", "§r§cReloading...");
			// если его нет, это баг, значит удаляем тэг
			if (!imho) return pl.removeTag("reload:" + XA.Entity.getTagStartsWith(pl, "reload:"));
			//выдаем пустое оружие вместо перезаряжающегося
			XA.Entity.getI(pl).setItem(
				imho[1],
				new ItemStack(Items.get(imho[0].typeId.replace("_empty", "") + "_empty"), 1, 0)
			);
			...
			//возвращаем пули
			XA.runCommand(`give "${pl.name}" b:l ${imho[0].getLore()[1].cc().split("/")[0]}`);
			//удаляем тэг
		...
		if (c == 0 && (await XA.Entity.hasItem(pl, "weapon.offhand", "b:l"))) {
			c = getItemsCountClear(pl, "b:l");
			bullsInOffhand = true;
		}
		if (gs[item?.typeId?.split(":")[1]]) {
			if (!bullsInOffhand && !(await XA.Entity.hasItem(pl, "weapon.offhand", "b:l"))) return;
		...
		c = ostB + c;
		pl.runCommand("scoreboard players set @s lockedtitle " + Math.floor(gun.reloadTime));
		pl.onScreenDisplay.setActionBar("§cReload");
		lore[0] = "§r§cReloading...";
		lore[1] = `§r§f${Number(XA.Entity.clearItems(pl, "b:l", gun.maxAmmo - ostB)) + Number(ostB)}/${
			gun.maxAmmo
		}`.replace("NaN", gun.maxAmmo);
		lore[2] = "§r§7" + c;
		pl.runCommand(`replaceitem entity @s slot.weapon.mainhand 0 ${item.typeId.replace("_empty", "")}_empty`);
		...
		setTickTimeout(() => {
			...
			const it = new ItemStack(Items.get(item.typeId.replace("_empty", "")), 1, 0);
			let lorec = [];
			lorec[0] = lore[1];
			it.setLore(lorec);
...
XA.objectives.push("clReTime");
//Обнуление триггера анимации
setTickInterval(() => {
	XA.runCommand("scoreboard players add @a[scores={mark=2,clReTime=1..}] clReTime -1");
	XA.runCommand("event entity @a[scores={mark=2,clReTime=0}] mark:clear");
}, 0);
...
setTickInterval(() => {
	for (const e of world.getDimension("overworld").getEntities()) {
		...
		e.nameTag = `с: ${ent.length + Number(e.nameTag.split(" ")[1])} ${e.nameTag.split(" ")[2]} ${e.nameTag
			.split(" ")[3]
			?.replace("NaN", 0)}`;
		const p = XA.Entity.getClosetsEntitys(e, 5, "minecraft:player").find((p) => p.hasTag("attacking"));

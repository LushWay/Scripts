...
world.events.entityHurt.subscribe((data) => {
	...
	if (!["fire", "fireworks", "projectile"].includes(damage.cause)) return;
	if (
	...
	let lastHit = false;
	if (
		data.hurtEntity.getComponent("minecraft:health").current - data.damage <=
		0
	)
		lastHit = true;
	...
	if (damage?.damagingEntity instanceof Player) {
		...
		if (lastHit) stats.kills.eAdd(damage.damagingEntity, 1);

...
function childrensToHelpText(player, command) {
  const childrens = []
  for (const children of command.sys.children.filter(e =>
...
function getParentType(command, init = true) {
  ...
  if (command.sys.depth === 0 || !command.sys.parent)
    ...
    return [
      `${parents[0]}§f ${curtype}`,
      description ?? parents[1] ?? '<нет описания>',
    ]
  }
}
...
help
  ...
  .executes((ctx, inputPage, commandsInPage) => {
    const avaibleCommands = Command.commands.filter(e =>
    ...
    ctx.reply(
      `${cv}─═─═─═§f Доступно: ${avaibleCommands.length}/${Command.commands.length} ${cv}═─═─═─═─`
    )
  })
...
function helpForCommand(ctx, commandName) {
  ...
  for (const [command, description] of childrensToHelpText(ctx.sender, cmd).map(
    e => getParentType(e)
  )) {
...
Command.getHelpForCommand = (command, ctx) =>

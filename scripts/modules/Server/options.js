...
new WorldOption("chat:range", "Радиус для затемнения сообщений дальних игроков (число)", true);

new PlayerOption(
	"chat:highlightMessages",
	"Если включено, вы будете видеть свои сообщения в чате так: §l§6Я: §fСообщение§r"
);
new PlayerOption("chat:sound:msgl:disable", "Отключает звук сообщений дальних игроков в чате");
new PlayerOption("chat:sound:msgn:disable", "Отключает звук сообщений ближних игроков в чате");
// new PlayerOption(
//   "chat:sound:@:disable",
...
new WorldOption("lock:nether", "Если эта настройка включена, порталы в незер будут автоматически ломаться");
new WorldOption("chat:ranks:disable", "Выключает ранги в чате");
new WorldOption("server:bowhit", "§aВключает§7 звук попадания по энтити из лука");
if (wo.Q("server:bowhit")) {
	new PlayerOption("pvp:bowhitsound", "§aВключает§7 звук попадания по энтити из лука");
	new PlayerOption("pvp:bowhittitle", "§aВключает§7 титл попадания по энтити из лука");
}
new WorldOption("server:pvpmode:enable", "§aВключает§7 возможность входа в пвп режим (блокировка всех тп команд)§r");
if (wo.Q("server:pvpmode:enable")) {
	new PlayerOption("title:pvpmode", "Выводит надпись с временем пвп режима над хотбаром");
	new WorldOption("server:pvpmode:cooldown", "Определяет время пвп режима", true);
}
new WorldOption("timer:enable", "Включает таймер игрового времени");
new WorldOption("zone:center", "x y", true);
new WorldOption("imports:clean", "Делает сообщения об импортах чище (включите при частой перезагрзуке)");
new WorldOption("import:wb", "Включает импорт WorldBuilder'a");
new WorldOption("debug:menu", "Выключает дебаг");

new WorldOption("server:spawn", "Включает команду /hub, пункт в меню и приветствие.\n\n(требуется перезагрузка)");
if (wo.Q("server:spawn")) {
	...
	new PlayerOption("title:spawn:enable", 'Переводит надпись "Перемещено" в зону над хотбаром');
	...
	world.events.playerJoin.subscribe((PlayerJoinEvent) => {
		...
		if (!po.Q("title:join:disable", pl) && XA.Entity.getScore(pl, "pvp") == 0)
			pl.addTag("joinedAt:" + pl.location.x + " " + pl.location.y + " " + pl.location.z);
	});
	new WorldOption("joinform:body", "Сообщение при первом входе. Параметры: $name, $time", true);
}

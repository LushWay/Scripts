...
system.runPlayerInterval(
  (player) => {
    ...
    const data = DB.get(currentRegion.permissions.owners[0]);
...
MENU.OnOpen = (player) => {
  ...
  const playerRegion = CubeRegion.getAllRegions().find(
    (e) => e.key === regionID
  );
  if (!playerRegion) {
    player.tell(
  ...
  if (currentRegion) {
    ...
    if (inOwnRegion) {
      ...
      if (regionOwnerName) {
        ...
        const rawArchive = DB.get("ARCHIVE:" + currentRegion.key).archiveOwners;
        const archive = Array.isArray(rawArchive) ? rawArchive : [];
        regionOwnerName = XEntity.getNameByID(archive[0]);

        add("§eЭто площадка была заархивирована");
        add(
          "§3Строители: §f" + archive.map(XEntity.getNameByID).join("§r§3, §f")
        );
      }
    }
  ...
  menu.addButton("Площадки", () => {
    ...
    for (const reg of CubeRegion.getAllRegions().filter(
      ...
      toPlatform(
        reg,
        XEntity.getNameByID(reg.permissions.owners[0]) ?? "NoName"
      );
    }
    addBackButton(form);
    form.show(player);
    ...
    if (inOwnRegion) {
      ...
      menu.addButton("Строители", () => {
        ...
        for (const id of currentRegion.permissions.owners.slice(1)) {
          let playerName = XEntity.getNameByID(id);

  ...
  q => {
        ...
        if (firstTime)
          system.delay(() => {
            this.player
              ...
              .container.addItem(LEARNING.START_AXE)
          })
        ...
        return new Temporary(({ world }) => {
          world.afterEvents.playerBreakBlock.subscribe(
            ({ player, brokenBlockPermutation }) => {
              ...
              if (
                !LEARNING.START_AXE_BREAKS.includes(
    ...
    q.dynamic({
      ...
      activate() {
        ...
        if (!key && !airdrop.chestMinecart)
          return this.error('Не удалось вызвать аирдроп')
        ...
        this.quest.steps(this.player).createTargetMarkerInterval({
          ...
          interval() {
            ...
            for (const vector of Vector.foreach(
              this.place,
              Vector.add(this.place, { x: 0, y: -10, z: 0 })
            ))
              try {
...
export const LEARNING = {
  LEARNING_Q,
  START_AXE: new ItemStack(MinecraftItemTypes.WoodenAxe).setInfo(
    '§r§6Начальный топор',
    'Начальный топор'
  ),
  /** @type {string[]} */
...
world.afterEvents.playerBreakBlock.subscribe(event => {
  if (
    LEARNING.START_AXE_BREAKS.includes(event.brokenBlockPermutation.type.id)
  ) {
    event.block.setType(

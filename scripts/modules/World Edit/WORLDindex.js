import { BlockLocation, BlockRaycastOptions, MinecraftBlockTypes, Player, world } from "@minecraft/server";
//import { SessionBuild } from "./modules/builders/SessionBuilder.js";
import { setTickInterval, XA } from "xapi.js";
import { PlayerOption, po } from "../../lib/Class/XOptions.js";
...
world.events.blockPlace.subscribe((data) => {
	...
	if (blocks.find((e) => e.split(".")[1])) {
		...
		const block = MinecraftBlockTypes.get("minecraft:" + blocks[~~(Math.random() * blocks.length)]);
		if (!block) return;
...
setTickInterval(() => {
	for (const p of world.getPlayers()) {
		if (
			...
			(po.Q("mobile", p) && p.hasTag("using_item") && XA.Entity.getHeldItem(p)?.typeId == "we:s")
		) {
			const i = XA.Entity.getHeldItem(p);
			const lore = i.getLore();
			...
			if (lore[4] && lore[0] == "§9Adv") {
				...
				const O = Number(`${Z}${lore[4].split(" ")[3]}`);
				new Fill(
					new BlockLocation(Math.floor(p.location.x), Math.floor(p.location.y + H), Math.floor(p.location.z)),
					new BlockLocation(Math.floor(p.location.x), Math.floor(p.location.y + O), Math.floor(p.location.z)),
					B,
					RB ?? "any"
				);
			}
		}
		if (XA.Entity.getHeldItem(p)?.typeId == "we:brush" && !po.Q("worldBuilder:NoBrushView", p)) {

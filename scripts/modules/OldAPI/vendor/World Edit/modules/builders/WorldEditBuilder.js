...
export class WorldEditBuilder {
  ...
  drawSelection() {
    ...
    const ent2 = SA.Models.world.getEntityAtPos(
      this.pos2.x,
      this.pos2.y,
      this.pos2.z
    );
  ...
  copy() {
    try {
      ...
      const opt = SA.Build.chat.newCommand(
  ...
  async fillBetween(block, data = -1, replaceMode, replaceBlock, rbData) {
    ...
    if (errors) {
      return new Return(false, 0, {
        fillCount: blocks,
        statusMessage: `§4► §c${errors}§f\\§a${all}§7 Заполнено с ошибкой, §f${blocks} §7(§f~${endTime}§7 мс). ${
          replaceMode

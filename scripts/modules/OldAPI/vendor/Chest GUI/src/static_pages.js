import { db, OPTIONS, wo, WORLDOPTIONS } from "../../../app/Models/Options.js";
...
function perenos(string) {
  ...
  for (let word of string.split(" ")) {
    count = count + SA.Utilities.format.clearColors(word).length;
    if (
      maxC <= count &&
      word != "Экспериментальная" &&
      word != "настройка" &&
      word != "▲"
    ) {
      output = output + " \n" + word;
...
const def = new Page(`set`, 54, "default")
  ...
  .createItem(
    ...
    0,
    "page:lich",
    "§r§5§l☼ §fНастройки §5§l☼",
    ["\n§r§7Настройки игрока"]
  )
  .createItem(40, "minecraft:barrier", 1, 0, "close", "§rЗакрыть GUI");
if (wo.Q("server:spawn")) {
  def.createItem(22, "minecraft:beacon", 1, 0, "Atp:spawn", "На спавн", [
    "",
    "§r§7Переносит на спавн",
  ]);
  def.createItem(20, "minecraft:tnt", 1, 0, "Atp:anarch", "Анархия", [
    "",
    "§r§7Переносит на анахрию",
  ]);
  def.createItem(
    24,
    ...
    "Atp:minigames",
    "Миниигры",
    ["", "§r§7Переносит на спавн миниигр"]
  );
}
if (wo.Q("timer:enable"))
  def.createItem(42, "reinforced_deepslate", 1, 0, "stats", "Статистика", [
    "",
    "§r§7Открывает меню\nсо статистикой",
  ]);
...
export let auxa = new Page(`worldsett`, 54, "default")
  .createItem(45, default_item, 1, 0, "set", " ")
  .createItem(46, default_item, 1, 0, "set", " ")
  .createItem(47, "minecraft:shulker_box", 1, 0, "page:items", "§r§lПредметы", [
    "",
    "§r§7Сохраненные предметы",
  ])
  .createItem(52, default_item, 1, 0, "set", " ")
  .createItem(53, default_item, 1, 0, "set", " ")
  .createItem(48, "minecraft:coral", 1, 3, "sc:clear", "§r§lОчистить все", [
    "",
    "§r§7Очищает все настройки",
  ])
  .createItem(49, "minecraft:barrier", 1, 0, "close", "§rЗакрыть")
  .createItem(50, "minecraft:kelp", 1, 0, "sc:clear0", "§r§lОчистить 0", [
    "",
    "§r§7Очищает выключенные настройки",
  ])
  .createItem(51, "minecraft:lectern", 1, 0, "page:perm", "§r§lРазрешения", [
    "",
    "§r§7Меню разрешений",
  ]);
SA.Utilities.time.setTickTimeout(() => {
  ...
  for (let s of WORLDOPTIONS) {
    if (s.name.startsWith("perm:")) {
      if (!db.has(s.name)) db.set(s.name, "");
      continue;
    }
    if (s.text) {
      auxa.createItem(

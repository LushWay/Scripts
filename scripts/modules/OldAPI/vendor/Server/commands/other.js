...
cos
  ...
  .executes((ctx, id) => {
    ctx.reply(
      "Зарегано под айди: " +
        SA.tables.i.add(XA.Entity.getHeldItem(ctx.sender), id)
    );
  });
cos
  .literal({ name: "get" })
...
new XA.Command({
  ...
  if (XA.Entity.hasItemm(ctx.sender, "mcbehub:gui")) {
...
new XA.Command({
  ...
  if (XA.Entity.hasItemm(ctx.sender, "sa:a")) {

...
const genb = new XA.Command({
  ...
  ctx.reply(
    `§a► §f${XA.Entity.getHeldItem(ctx.sender)?.typeId} ${
      XA.Entity.getHeldItem(ctx.sender)?.data
    } ${
      XA.Entity.getHeldItem(ctx.sender)?.nameTag
        ? XA.Entity.getHeldItem(ctx.sender)?.nameTag
        : ""
    }`
  );
...
genb.literal({ name: "a", description: "добав" }).executes((ctx) => {
  ...
  blocks.push(
    `${XA.Entity.getHeldItem(ctx.sender)?.typeId}.${
      XA.Entity.getHeldItem(ctx.sender)?.data
    }`
  );
...
genb
  .literal({ name: "rot", description: "Выдает ротатион" })
  .executes((ctx) => {

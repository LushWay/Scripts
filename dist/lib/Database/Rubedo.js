import { DynamicPropertiesDefinition, Entity, EntityTypes, ItemStack, MinecraftItemTypes, world, } from "@minecraft/server";
...
export class Database {
    static initAllTables() {
        ...
        return Promise.all(Object.values(this.instances).map((table) => {
            const initing = table.init();
            initing.catch((err) => DisplayError(err, 0, [`${table.tableName} init`]));
            return initing;
        }));
    }
    /**
    ...
    constructor(tableName) {
         ...
         */
        this.MEMORY = null;
        if (tableName in Database.instances)
            return Database.instances[tableName];
    ...
    async save() {
        if (!this.MEMORY)
            throw new Error(`Cannot save data on table '${this.tableName}' before initing them. Add await to world load to solve that`);
        /**
    ...
    get(key) {
        if (!this.MEMORY)
            throw new Error("Get: Entities not loaded!");
    ...
    keys() {
        if (!this.MEMORY)
            throw new Error("Keys: Entities not loaded!");
        // @ts-expect-error
    ...
    values() {
        if (!this.MEMORY)
            throw new Error("Values: Entities not loaded!");
    ...
    has(key) {
        if (!this.MEMORY)
            throw new Error("Has: Entities not loaded!");
    ...
    get collection() {
        if (!this.MEMORY)
            throw new Error("Collection: Entities not loaded!");
        return this.MEMORY;
    }
    /**
    ...
    delete(key) {
        if (!this.MEMORY)
            throw new Error("Delete: Entities not loaded!");
    ...
    clear() {
        if (!this.MEMORY)
            throw new Error("Clear: Entities not loaded!");
        Object.assign(this.MEMORY, {});
        return this.save();
    }
}
Database.tablesInited = false;

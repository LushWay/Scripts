import { Entity, ItemUseOnEvent, Player, world, World, } from "@minecraft/server";
...
addMethod(Player.prototype, "applyDash", (target, horizontalStrength, verticalStrength) => {
 ...
 * @returns
 */
function teleport({ original, args: [location, dimension, xRot, yRot, keepVelocity], context, }) {
    if (typeof xRot === "undefined" || typeof yRot === "undefined") {
        const rotation = context.getRotation();
        xRot = rotation.x;
        yRot = rotation.y;
    }
    keepVelocity ?? (keepVelocity = false);
...
World.prototype.events.beforeItemUseOn.subscribe((data) => {
    data;
});
Reflect.defineProperty(ItemUseOnEvent.prototype, "blockLocation", {
    get() {
        this.location ?? (this.location = this.getBlockLocation());

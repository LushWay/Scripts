...
export function sendCallback(cmdArgs, args, event, baseCommand) {
    ...
    for (const [i, arg] of args.entries()) {
        if (arg.sys.type.name.endsWith("*"))
            continue;
        if (arg.sys.type instanceof LocationArgumentType) {
            argsToReturn.push(parseLocationAugs([cmdArgs[i], cmdArgs[i + 1], cmdArgs[i + 2]], event.sender) ?? event.sender.location);
            continue;
        }
        if (arg.sys.type instanceof LiteralArgumentType)
            continue;
    ...
    handle(() => lastArg.sys.callback(new CommandContext(event, cmdArgs), ...argsToReturn), "Command");
}

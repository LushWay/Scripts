...
export class ModalForm {
    ...
    async show(player, callback) {
        ...
        if (response === false || !(response instanceof ModalFormResponse))
            return;
        handle(() => callback(new FormCallback(this, player, callback), ...response.formValues.map((v, i) => (this.args[i].type === "dropdown" ? this.args[i].options[v] : v))), null, ["ModalFormCallback"]);
    }
}

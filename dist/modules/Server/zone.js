...
setTickInterval(() => {
    ...
    for (const p of players) {
        ...
        if (l.x >= rmax.x && l.x <= rmax.x + 10 && ztrue)
            ret(p, true, rmax);
        if (l.x >= rmax.x - 10 && l.x <= rmax.x && ztrue)
            pret(p, true, rmax);
        if (l.z >= rmax.z && l.z <= rmax.z + 10 && xtrue)
            ret(p, false, rmax);
        if (l.z >= rmax.z - 10 && l.z <= rmax.z && xtrue)
            pret(p, false, rmax);
        if (l.x <= rmin.x && l.x >= rmin.x - 10 && ztrue)
            ret(p, true, rmin, true);
        if (l.x <= rmin.x + 10 && l.x >= rmin.x && ztrue)
            pret(p, true, rmin);
        if (l.z <= rmin.z && l.z >= rmin.z - 10 && xtrue)
            ret(p, false, rmin, true);
        if (l.z <= rmin.z + 10 && l.z >= rmin.z && xtrue)
            pret(p, false, rmin);
    }
}, 0, "zone");

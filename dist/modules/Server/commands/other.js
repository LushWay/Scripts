...
kit.string("name", true).executes((ctx, name) => {
    ...
    if (ALLkits.length < 1)
        return ctx.reply("§сНет китов");
    const kits = ALLkits.filter((e) => e.split(":")[0] == "any" || ctx.sender.hasTag(e.split(":")[0]));
    if (kits.length < 1)
        return ctx.reply("§сНет доступных китов");
    let kkits = [], avaible = {}, unavaible = [];
    for (const kit of kits) {
        const cooldown = Number(ctx.sender
            .getTags()
    ...
    if (!name)
        return ctx.reply("§fДоступные киты: \n  §7" + kkits.join("\n  "));
    if (!avaible[name]) {
        if (unavaible.find((e) => e.split(":")[0] == name))
            return ctx.reply(`§cКит §6${name}§c был взят недавно. §b${unavaible.find((e) => e.split(":")[0] == name).split(":")[1]}`);
...
kit
    ...
    if (!n)
        return ctx.reply(`§cКита с названием '§f${name}§c' не существует. \n§fДоступные киты: \n  §7` +
...
kit
    ...
    for (const [i, k] of kit.entries()) {
        inv.setItem(i, new ItemStack(ItemTypes.get(k.split(" ")[0].includes(":")
            ? k.split(" ")[0]
            : "minecraft:" + k.split(" ")[0]), Number(k.split(" ")[1])));
    }

...
var _inventory_instances, _inventory_gamerules, _inventory_setZone;
import { ItemStack, MinecraftBlockTypes, MinecraftItemTypes, Player, Vector, world, } from "@minecraft/server";
...
function tp(player, pos, place, resultActionbar = false, obj, text, slow_falling, tpAnimation = true) {
    ...
    if (slow_falling)
        player.runCommandAsync("effect @s slow_falling 17 1 true");
    let befplace;
    if (obj && obj.on) {
...
function getPlace(place, text) {
    let P = place, C = text, rot;
    if (place == "anarch")
        (P = "§cАнархия"), (C = "4");
    if (place == "spawn")
        (P = "§aСпавн"), (C = "2"), (rot = "0 0");
    if (place == "br")
        (P = "§6Батл рояль"), (C = "e"), (rot = "0 0");
...
class inventory {
    ...
    async saveInv(i, pl) {
        ...
        if (i != invs.anarch || inv.size == inv.emptySlotsCount)
            return;
        //console.warn("Сохранение инвентаря, старт");
        __classPrivateFieldGet(this, _inventory_instances, "m", _inventory_gamerules).call(this, true);
        const zone = this.savezones.find((e) => world.getDimension("overworld").getEntitiesAtBlockLocation(e).length < 1);
        if (!zone)
            return world.say("§cError");
        __classPrivateFieldGet(this, _inventory_instances, "m", _inventory_setZone).call(this, zone);
        pl.teleport({ x: zone.x, y: zone.y, z: zone.z }, world.getDimension("overworld"), 0, 0, false);
        await sleep(10);
        pl.addTag("saving");
        const name = pl.name;
        pl.kill();
        sleep(10).then((e) => {
            XA.runCommandX(`structure save ${this.structureName.replace("$name", pl.name)} ${zone.x} ${zone.y} ${zone.z} ${zone.x} ${zone.y + 1} ${zone.z} true disk false`);
        ...
        __classPrivateFieldGet(this, _inventory_instances, "m", _inventory_gamerules).call(this, false);
    }
    /**
     *
     * @param {number} i
    ...
    async loadInv(i, pl) {
        if (i != invs.anarch) {
            ...
            XA.runCommandX(`replaceitem entity ${pl.name} slot.hotbar 4 mcbehub:gui 1 0 {"item_lock":{"mode":"lock_in_slot"}}`);
        }
        else {
            //console.warn("Загрузка инвентаря анархии, старт");
            const zone = this.savezones.find((e) => world.getDimension("overworld").getEntitiesAtBlockLocation(e).length <
                1);
            if (!zone)
                return world.say("§cError");
            __classPrivateFieldGet(this, _inventory_instances, "m", _inventory_setZone).call(this, zone);
            pl.teleport({ x: zone.x, y: zone.y, z: zone.z }, world.getDimension("overworld"), 0, 0, false);
            XA.runCommandX(`clear ${pl.name}`);
            XA.runCommandX(`structure load ${this.structureName.replace("$name", pl.name)} ${zone.x} ${zone.y} ${zone.z} 0_degrees none true false`);
...
_inventory_instances = new WeakSet(), _inventory_gamerules = function _inventory_gamerules(IsSet) {
    if (IsSet) {
        this.save = [];
        ...
        this.save.forEach((g) => XA.runCommandX(`gamerule ${g.split(" = ")[0]} ${g.split(" = ")[1]}`));
}, _inventory_setZone = function _inventory_setZone(bl) {
...
export function Atp(player, place, ignore, setDefaultInventory) {
    ...
    if ((currentInventory == placeIndex ||
        ...
        try {
            if (!setDefaultInventory)
                player.runCommandAsync("testfor @s[m=!c]");
            inv.saveInv(currentInventory, player).then(() => inv.loadInv(placeIndex, player).then(() => {
                player.runCommandAsync(`scoreboard players set @s ${objective[0]} ${placeIndex}`);
                tp(player, pos, place, settings.title, obj, null, air, (!setDefaultInventory && inve.size == inve.emptySlotsCount) ||
                    place == "currentpos");
            }));
        }
        catch (e) {
...
new XA.Command({
    ...
    const block = ctx.sender.dimension.getBlock(XA.Utils.floorVector(ctx.sender.location).offset(0, -4, 0));

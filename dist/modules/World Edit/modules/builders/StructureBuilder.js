...
export class Structure {
    ...
    async save() {
        ...
        for (const region of regions) {
            ...
            const result = await XA.runCommandX(`structure save "${name}" ${pos1.x} ${pos1.y} ${pos1.z} ${pos2.x} ${pos2.y} ${pos2.z} memory`, { showError: true });
            all++;
            if (result > 0) {
                this.files.push({
                    name: name,
        ...
        */
        ...
        if (errors > 0)
            throw new Error(`§c${errors}§f/§a${all}§f не сохранено.`);
    }
    async load() {
        let errors = 0;
        let all = 0;
        for (const file of this.files) {
            const result = await XA.runCommandX(`structure load "${file.name}" ${file.pos1.x} ${file.pos1.y} ${file.pos1.z}`, {
                showError: true,
            });
            all++;
            if (result === 0) {

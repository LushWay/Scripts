...
const shovelCMD = new XA.Command({
    ...
    if (!радиус)
        return ctx.reply("§c" + радиус);
    const brush = new ItemStack(ItemTypes.get(`we:s`));
    if (радиус > 6)
        return ctx.reply("§c► Зачем тебе такой БОЛЬШОЙ?)");
    if (высота > 10)
        return ctx.reply("§c► Зачем тебе такой БОЛЬШОЙ?)");
    let bblocks;
    ...
    ctx.reply(`§a► §rПолучена лопата ${blocks} блоками, высотой ${высота}, радиусом ${радиус} и заполняемыми блоками ${replaceBlocks}`);
});
shovelCMD
    .literal({
    name: "height",

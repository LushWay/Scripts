...
set
    ...
    if (!blockIsAvaible(block, ctx.sender))
        return;
    if (other && !blockIsAvaible(other, ctx.sender))
        return;
    const time = timeCaculation(WorldEditBuild.pos1, WorldEditBuild.pos2);
    if (time >= 0.01)
        ctx.reply(`§9► §rНачато заполнение, которое будет закончено приблизительно через ${time} сек`);
...
set.executes((ctx) => {
    ...
    new ModalForm("Заполнить")
        ...
        if (time >= 0.01)
            ctx.reply(`§9► §rНачато заполнение, которое будет закончено приблизительно через ${time} сек`);
        WorldEditBuild.fillBetween(block, blockData, mode !== "none" ? mode : "", other, otherData).then((result) => ctx.reply(result));
    });
});
/**

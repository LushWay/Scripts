import { MinecraftBlockTypes, MinecraftDimensionTypes, Player, } from "@minecraft/server";
...
export function teleportToRegion(player, region) {
    ...
    player.teleport({ x: region.from.x, y: squarePlace + 3, z: region.from.z }, XA.dimensions.overworld, rotation.x, rotation.y);
}
/**
 * @param {Player} player
 * @param {string} text
...
export async function ClearRegion(player, Pregion) {
    ...
    const end = setTickInterval(() => {
        ...
        time++;
        if (time % 10 === 0)
            spining = spining.slice(1).concat(spining.shift());
    }, 0, "ResetSquare_ProgressActionbar");
    ...
    for (const loc of e) {
        ...
        if (block.typeId === MinecraftBlockTypes.air.id)
            continue;
...
export async function fillRegion(from, to) {
    ...
    for (const loc of XA.Utils.safeBlocksBetween(firstLoc.above(), secondLoc.above())) {
        await exec();
...
export async function findFreePlace() {
    ...
    while (!from) {
        ...
        const alreadyExist = Region.blockLocationInRegion({ x: center.x, y: 0, z: center.z }, MinecraftDimensionTypes.overworld);
        if (alreadyExist) {

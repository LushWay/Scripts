...
world.events.entitySpawn.subscribe((data) => {
    const region = Region.blockLocationInRegion(data.entity.location, data.entity.dimension.id);
    if (!region && GLOBAL_ALLOWED_ENTITIES.includes(data.entity.typeId))
        return;
...
setPlayerInterval((player) => {
    ...
    if (loc.y < TP_Y)
        player.teleport({ x: loc.x, y: TP_TO, z: loc.z }, player.dimension, rotation.x, rotation.y);
}, 0, "underground effects");

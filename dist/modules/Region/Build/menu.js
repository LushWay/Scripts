...
CONFIG_MENU.menu = (player) => {
    ...
    const inOwnRegion = current_region &&
        current_region.permissions.owners[0] === Pregion.permissions.owners[0];
    const isRegionBuilder = current_region && current_region.permissions.owners.includes(player.id);
    let body = "";
    ...
    menu.addButton("Площадки", null, () => {
        const form = new ActionForm("Доступные площадки", "§3Здесь вы можете телепортироваться на любую площадку, где можете строить");
        /**
         *
         * @param {Region} region
         * @param {string} name
        ...
        for (const reg of Region.getAllRegions().filter((e) => e.permissions.owners.includes(player.id) && e.key !== Pregion.key)) {
    ...
    if (current_region) {
        if (inOwnRegion) {
            menu.addButton("Строители", null, () => {
                ...
                for (const id of current_region.permissions.owners.slice(1)) {
                    ...
                    form.addButton(playerName, null, () => {
                        prompt(player, "§cВы точно хотите удалить игрока §r" +
                            playerName +
                            "§r§c из строителей вашего региона?", "ДА", () => {
            ...
            menu.addButton(`Запросы редактирования${reqs > 0 ? `: §c${reqs}!` : ""}`, null, () => {
                const newmenu = new ActionForm("Запросы редактирования", "§3В этом меню вы можете посмотреть запросы на редактирование площадки, отправление другими игроками");
                for (const ID of req.reqList) {
                    const name = XA.Entity.getNameByID(ID);
                    newmenu.addButton(name, null, () => {
                        prompt(player, "Принимая запрос на редактирование, вы даете игроку право редактировать ваш регион.", "Принять", () => {
                            const requestedPlayer = XA.Entity.fetch(ID);
                            if (requestedPlayer) {
            ...
            menu.addButton("Перейти на новую", null, () => {
                ...
                if (CD.isExpired())
                    prompt(player, "§fВы уверены, что хотите перейти на новую площадку? После этого настощая площадка будет архивирована, и §cвернуть её§f будет уже §cнельзя§f. Это действие можно совершать раз в 24 часа.", "§cДа, перейти", () => {
            ...
            menu.addButton("§cОчистить", null, () => {
                ...
                if (CD.isExpired())
                    prompt(player, "§fВы уверены что хотите §cбезвозвратно§f очистить площадку§f? Это действие §cнельзя отменить.", "§cВсе равно очистить§r", async () => {
                        CD.update();
                        const end = await ClearRegion(player, Pregion);
                        await handle(() => fillRegion(Pregion.from, Pregion.to));
                        end();
                    }, "Отмена, не очищайте", () => menu.show(player));
            });
        }
            ...
            menu.addButton("Запросить разрешение", null, () => {
                ...
                if (CD.isExpired())
                    prompt(player, "Владельцу площадки будет отправлен запрос на редактирование. Если он его примет, вы сможете строить на его площадке.", "Запросить", () => {
                        CD.update();
                    ...
                    }, "Отмена", () => menu.show(player));
            });
        }
        if (IS(player.id, "admin")) {
            menu.addButton("§cУдалить площадку", null, () => {
                prompt(player, `§cВы действительно хотите удалить площадку игрока §r§f${regionOwnerName}?`, "§cДа", async () => {
                    const end = await ClearRegion(player, current_region);
                    current_region.forEachOwner((player) => player.tell(`§cРегион с владельцем §f${regionOwnerName}§r§c был удален`));
                    current_region.delete();
                    end();
                }, "НЕТ!", () => menu.show(player));
            });
        }
    }
    return menu;

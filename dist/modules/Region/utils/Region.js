...
export class Region {
    ...
    static getAllRegions() {
        if (REGIONS_HAVE_BEEN_GRABBED)
            return REGIONS;
        const regions = TABLE.values().map((region) => new Region(region.from, region.to, region.dimensionId, region.permissions, region.key, false));
        regions.forEach((r) => {
            REGIONS.push(r);
        });
        return regions;
    ...
    forEachOwner(callback) {
        ...
        if (this.permissions.owners.length < 1)
            return;

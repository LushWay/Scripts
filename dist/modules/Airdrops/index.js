...
setTickInterval(() => {
    for (const ent of world.getDimension("overworld").getEntities(q)) {
        ...
        if (!cl)
            return ent.removeTag("держит");
        const block = ent.dimension.getBlock(XA.Utils.floorVector({
            x: ent.location.x,
            y: ent.location.y - 4,
            z: ent.location.z,
        }));
        const block2 = ent.dimension.getBlock(XA.Utils.floorVector({
            x: ent.location.x,
            y: ent.location.y - 1,
            z: ent.location.z,
        }));
        ...
        for (const clo of cl) {
            ...
            if (block.typeId == "minecraft:air")
                clo.teleport({ x: ent.location.x, y: ent.location.y - 3, z: ent.location.z }, ent.dimension, rotation.x, rotation.y);
            break;
        }
    }
}, 0, "airDrop"); /*

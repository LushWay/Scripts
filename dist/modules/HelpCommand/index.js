...
function childrensToHelpText(command) {
    ...
    for (const children of command.sys.children) {
        if (children.children.length < 1)
            ...
            _.push(...childrensToHelpText(children));
    }
    return _;
}
/**
...
help
    ...
    const avaibleCommands = XCommand.COMMANDS.filter((e) => e.sys.data.requires(ctx.sender));
    ...
    for (const command of path) {
        const q = "§f-";
        let c = `${cv}§r ${q}${command.sys.data.name} §o§7- ${command.sys.data.description
...
help.string("commandName").executes((ctx, commandName) => {
    const cmd = XCommand.COMMANDS.find((e) => e.sys.data.name == commandName ||
        e.sys.data?.aliases?.includes(commandName));
    if (!cmd)
        return commandNotFound(ctx.sender, commandName);

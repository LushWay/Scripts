        ...
        try {
            ...
            this.events = {
                tick: world.events.tick.subscribe(() => {
                    ...
                    if (this.time.sec <= 0)
                        this.time.min--, (this.time.sec = 59);
                    //Зона
                    ...
                    XA.runCommandX(`title @a[tag="br:inGame"] actionbar §6${this.players.filter((e) => e.hasTag("br:alive")).length} §g○ §6${this.time.min}:${`${this.time.sec}`.length < 2
                        ? `0${this.time.sec}`
                        : this.time.sec} §g○ §6${this.game.rad}`);
                    //Конец игры
                    if (this.time.min <= -1)
                        this.end("time");
                    if (this.players.filter((e) => e.hasTag("br:alive")).length <= 1)
                        this.end("last", this.players.find((e) => e && e.hasTag("br:alive")));
                }),
                ...
                buttonPush: world.events.buttonPush.subscribe((data) => {
                    if (!data.source.hasTag("br:alive") ||
     ...
     */
        ...
        if (reason === "time") {
            this.players.forEach((e) => {
                e.tell(XA.Lang.lang["br.end.time"](this.players
        ...
        for (const e of world.getPlayers()) {
            ...
            if (ingame)
                Atp(e, "br", { lock: true, pvp: true, quene: true });
        }

...
let minpl = 2, fulltime = 5, shorttime = 3;
/**
...
BATTLE_ROYAL_EVENTS.playerJoin.subscribe((player) => {
     ...
     */
    const pl = player;
    if (br.players.map((e) => e.name).includes(pl.name))
        return;
    if (br.game.started)
        return pl.onScreenDisplay.setActionBar(`§cИгра уже идет!`);
    if (quene[pl.name])
        return pl.onScreenDisplay.setActionBar(`§6${ks(quene).length}/${minpl} §g○ §6${br.quene.time}`);
    quene[pl.name] = true;
...
setTickInterval(() => {
    ...
    if (ks(quene).length >= minpl && ks(quene).length < 10) {
        if (!br.quene.open) {
            ...
            forEveryQuenedPlayer("random.levelup", `§7${ks(quene).length}/${minpl} §9Игроков в очереди! Игра начнется через §7${fulltime}§9 секунд.`);
        }
        ...
        if (br.quene.time >= 1 && br.quene.time <= shorttime) {
            let sec = "секунд", hrs = `${br.quene.time}`;

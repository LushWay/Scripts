...
setPlayerInterval((player) => {
    ...
    if (typeof at === "string") {
        ...
        const not_moved = player.location.x === pos[0] &&
        ...
        if (not_moved) {
            ...
            if (player.hasTag("on_ground")) {
                ...
                if (CONFIG_JOIN.subtitle)
                    options.subtitle = CONFIG_JOIN.subtitle;
                // Show...
...
function JOIN(player, data, messageType) {
    ...
    if (!data.learning)
        __JOIN_EMITTERS.PlayerJoin.emit(player);
    player.onScreenDisplay.clearTitle();
    const oldTag = data.name;
    if (oldTag === player.name)
        return;

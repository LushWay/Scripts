import { MinecraftBlockTypes, MolangVariableMap, system, world, } from "@minecraft/server";
import { DisplayError, handle, IS, setTickTimeout, sleep, toStr, XA, } from "xapi.js";
...
const tests = {
    ...
    3: async () => {
        ...
        async function findFreePlace() {
            ...
            while (!from) {
                tries++;
                if (tries >= 20)
                    await sleep(1), (tries = 0);
                const alreadyExist = regs.find((e) => betweenXYZ([e[0].x, 1, e[0].z], [e[1].x, -1, e[1].z], [center.x, 0, center.z]));
                if (alreadyExist) {
        ...
        const set = (pos) => XA.dimensions.overworld
    ...
    17: (ctx) => {
        ...
        if (typeof prop === "string")
            ctx.reply(prop.length);
        end();
    },
    18: (ctx) => {
        const region = Region.blockLocationInRegion(XA.Utils.floorVector(ctx.sender.location), ctx.sender.dimension.id);
    ...
    26(ctx) {
        ...
        for (const { x, y, z } of XA.Utils.safeBlocksBetween(pos1, pos2)) {
            ...
            if (q)
                ctx.sender.dimension.spawnParticle("minecraft:endrod", { x: x, y: y, z: z }, new MolangVariableMap());
        }
        end1();
    ...
    29(ctx) {
        ...
        world.say(toStr(XA.dimensions.overworld.getEntities({
    ...
    31(ctx) {
        const reg = Region.blockLocationInRegion(ctx.sender.location, ctx.sender.dimension.id);
        if (!reg)
            return ctx.error("No region!");
        reg.permissions.allowedEntitys.push(...ctx.args.filter((e) => e.includes(":")));
        reg.update();
    },
};
let bigdata = "";

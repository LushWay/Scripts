...
import { ScoreboardDB, scoreboardDisplayNames, scoreboardObjectiveNames } from 'lib/database/scoreboard'
import { form } from 'lib/form/new'
import { i18n, textTable } from 'lib/i18n/text'
import { capitalize } from 'lib/util'

new Command('stats').setDescription(i18n`Показывает статистику по игре`).executes(ctx => statsForm({}).command(ctx))

export const statsForm = form.params<{ targetId?: string }>((f, { player, params: { targetId = player.id } }) => {

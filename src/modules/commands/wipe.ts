...
interface RestorePoint {
	...
	scores: Pick<
		Player["scores"],
		"money" | "anarchyOnlineTime" | ScoreNames.GameModesStat
	>;
	db: Pick<
		PlayerDatabase,
		"inv" | "survival" | "unlockedPortals" | "quests" | "achivs"
	>;
}
...
function createRestorePoint(
	...
	try {
		if (player.database.inv !== "anarchy") {
			return player.fail(
				i18n.error`Вы не можете создать точку восстановления не находясь на анархии`
			);
		}

		if (
			![GameMode.adventure, GameMode.survival].includes(player.getGameMode())
		) {
			return player.fail(
				i18n.error`Вы не можете создать точку восстановления не находясь в выживании или приключении`
			);
		}
...
new Command("wipe")
	...
	.executes((ctx) => {
		...
		form((f) => {
			...
			f.body(
				i18n`Точки восстановления, нужные для тестирования\n\n§cЭНДЕР СУНДУК НЕ СОХРАНЯЕТСЯ`
			);
			...
			f.button(
				i18n`Точки восстановления других игроков`,
				otherPlayerRestorePoints
			);
...
function otherPlayerRestorePoints(player: Player, back?: NewFormCallback) {
	...
	new ArrayForm("Other players", players)
		.back(back)
		.button(([ownerId, db]) => {
			const title = i18n`${Player.name(ownerId) ?? ownerId} (${sizeOf(
				db.restorePoints
			)})`;
			return [
				title,
...
function restorePointMenu(
	player: Player,
	[ownerId, id]: RestorePointRef,
	...
	return form((f) => {
		...
		if (is(player.id, "techAdmin"))
			f.button("Log to the console", () =>
				console.log(JSON.stringify(restorePoint))
			);
...
function exitFromAllQuests(player: Player) {
	player.database.quests?.active.forEach((e) =>
		Quest.quests.get(e.id)?.exit(player)
	);
}

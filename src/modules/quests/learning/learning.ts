...
class Learning {
	...
	quest = new Quest(
		...
		(q, player) => {
			...
			q.counter(
				(current, end) => i18n.header`Добыто дерева: ${current}/${end}`,
				5
			)
					...
					if (firstTime) {
						...
						system.delay(() => {
							...
							player
								...
								?.setEquipment(EquipmentSlot.Offhand, Menu.itemStack);
						});
					}

					const trees = Object.values(MinecraftBlockTypes).filter((e) =>
						/log/i.exec(e)
					) as string[];

					ctx.world.afterEvents.playerBreakBlock.subscribe(
						({ player: ep, brokenBlockPermutation }) => {
							if (player.id !== ep.id) return;
							if (!trees.includes(brokenBlockPermutation.type.id)) return;

							logger.player(player)
								.info`Mined ${brokenBlockPermutation.type.id}`;
			...
			q.dynamic(i18n.header`Выйди под открытое небо`)
				.description(
					i18n`Деревья могут помешать. Выйди туда, где над тобой будет только небо`
				)
				.activate((ctx) => {
			...
			q.dynamic(i18n`Заберите все из сундука, упавшего с неба`)
				.description(
					i18n`Заберите все из упавшего с неба сундука. На него указывает компас`
				)
				.activate((ctx, firstTime) => {
						...
						if (i === 1) {
							...
							if (!airdrop.chest) {
								player.onScreenDisplay.setActionBar(
									i18n.error`Не удалось найти аирдроп\nИспользуйте .wipe чтобы перепройти обучение`.to(
										player.lang
									),
			...
			q.dynamic(i18n`Используй монеты в инвентаре`)
				.description(
					i18n`Возьми в руки монеты из инвентаря и используй, чтобы добавить на свой счет`
				)
				.activate((ctx) => {
					let money = 0;
					...
					ctx.world.afterEvents.itemUse.subscribe(({ source, itemStack }) => {
						if (source.id !== ctx.player.id || itemStack.typeId !== Items.Money)
							return;
			...
			q.breakCounter(
				(i, end) =>
					i18n.header`Спуститесь в шахту и добудьте камня: ${i}/${end}`,
				10
			)
			...
			q.counter(
				...
				.description(
					i18n`Вернитесь в шахту и вскопайте камень. Кажется, за ним прячется железо!`
				)
				.activate((ctx) => {
						...
						({
							...
							if (
								ctx.value === 0
									? id !== b.Stone
									: id !== b.IronOre &&
									  id !== MinecraftItemTypes.DeepslateIronOre
							)
								return;
			...
			q.dialogue(
				VillageOfMiners.guide,
				i18n`Шахтер зовет вас наверх, чтобы поговорить!`
			);
		}
	);

	learningLocation = location(
		this.quest.group
			.place("tp")
			.name(noI18n`Куда игроки будут тепаться при обучении`)
	);

	craftingTableLocation = location(
		this.quest.group.place("crafting table").name(noI18n`Верстак`)
	);
	...
	startAxeGiveCommand = createPublicGiveItemCommand(
		...
		(s) =>
			s.typeId === MinecraftItemTypes.WoodenAxe &&
			s.getDynamicProperty("startwand") === true,
	...
	constructor() {
		...
		actionGuard((player, region, ctx) => {
			...
			if (
				[...this.blockedOre.values()]
					.flat()
					.includes(Vec.string(ctx.event.block))
			) {

...
export class SafePlace {
	...
	private safeAreaLocation = locationWithRadius(
		this.group.place("safearea").name(noI18n`мирная зона`)
	);

	portalTeleportsTo = locationWithRotation(
		this.group
			.place("portal teleports to")
			.name(noI18n`портал телепортирует на`)
	);

	private portalFrom = location(
		this.group.place("portal from").name(noI18n`портал от`),
		undefined,
		true
	);

	private portalTo = location(
		this.group.place("portal to").name(noI18n`портал до`),
		undefined,
		true
	);
	...
	private createPortal(from: Vector3, to: Vector3) {
		...
		new Portal(this.group.id, start, end, (player) => {
			...
			if (!this.isUnlocked(player)) {
				player.success(
					i18n`Открыт новый портал! [Когда-нибудь здесь будет анимация...]`
				);
	...
	private createSafeArea(location: Vector3Radius) {
		...
		this.safeArea = SafeAreaRegion.create(
			new SphereArea(
				{ center: location, radius: location.radius },
				"overworld"
			),
			{
...
const notUnlockedPortalCost = ErrorCost(
	i18n`Портал в это поселение не разблокирован`
);
...
const portalMenuOnce = debounceMenu(function portalMenu(
	...
	return new ArrayForm(
		...
		.description(message)
		.button((place) => {
			const cost = place.unlocked
				? new MultiCost().money(1000)
				: notUnlockedPortalCost;
			const [name, , callback] = Product.create()
				...
				.onBuy(() => {
					if (place.location.valid)
						Portal.teleport(player, place.location, { title: "" });

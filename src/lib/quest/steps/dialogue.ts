...
export function QSDialogue(
  this: PlayerQuest,
  npc: Npc,
  text = i18n`Вас ждет ${i18n.accent`${npc.name}`}`,
  cancel = false,
) {
...
export function QSDialogueOverride(this: PlayerQuest, npc: Npc, text = i18n`Вас ждет ${i18n.accent`${npc.name}`}`) {
  return {
    body: (body: string) => ({
